...
\title[Radial metallicity gradients in NIHAO]{More than just a line: Shining light on the radial metallicity gradient with a NIHAO Milky-Way analogue simulation}
...
% Results
While a global linear fit traces the gradient well, we find it to be too flat in the inner galaxy and too steep in the outer galaxy. Fitting a flattening, second order quadratic function, however, provides a superior fit down to the smallest radial steps, where less regular effects (in the radial dimension) take over. These are identified as \SB{spiral arms or warp?} when traced across the Galactic position. The simulation further exhibits a steadily increasing spread in [Fe/H] of the young stars from 0.01 dex in the inner galaxy to 0.14 dex in at a radius 20 kpc coinciding with decreasing stellar density and increased flaring of stars towards higher heights with little correlation across the small age range of 1 Gyr.
% Conculsions
Conclusions: \\
\end{abstract}
...
\begin{itemize}
    ...
    \item In particular, we note that when fitting piece-wise linear gradients, we identify quite a significant curvature when going to smaller radial scales.
    \item When directly comparing an inner and outer radius fit, such as between $R_\mathrm{Gal.} = 5-10\,\mathrm{kpc}$ (thick grey line in Fig.~\ref{fig:radial_range_impact}) and $R_\mathrm{Gal.} = 10-15\,\mathrm{kpc}$ (thick black line in Fig.~\ref{fig:radial_range_impact}), we note a significant change, similar to previous estimates of the Milky Way \citep[e.g.][]{Lemasle2008}. In our case the gradient estimate changes from \input{tex_text/linear_gradient_5_10_kpc.tex} to \input{tex_text/linear_gradient_10_15_kpc.tex}.
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figures/radial_range_impact.png}
    \caption{Impact of different coverage in galactocentric radius when fitting a linear radial metallicity gradient to young stars. For better contrast, the global fit is subtracted from the local gradient estimates and each line is colored by the gradient slope with a color scale centered around the global fit slope.}
    \label{fig:radial_range_impact}
\end{figure}

\subsubsection{Higher-order gradients}
...
\SB{Guess we better test if the quadratic function is indeed a better fit than the 2 linear functions?}
...
\begin{itemize}
    \item the change of scatter from the innermost radii to the outermost (see Fig.~\ref{fig:global_r_feh_fit}c). We see a steady increase in $1-\sigma$ spread of \input{tex_text/fe_h_spread_at_R_0kpc} at $R_\mathrm{Gal.} = 0.25 \pm 0.25\,\mathrm{kpc}$, \input{tex_text/fe_h_spread_at_R_8kpc} at $R_\mathrm{Gal.} = 8.25 \pm 0.25\,\mathrm{kpc}$, to \input{tex_text/fe_h_spread_at_R_19kpc} at $R_\mathrm{Gal.} = 19.5 \pm 0.25\,\mathrm{kpc}$.
\end{itemize}
