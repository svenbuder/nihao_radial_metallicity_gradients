...
\begin{figure}
    ...
    \caption{Global fits and deviation to the radial metallicity gradient $R-\mathrm{[Fe/H]}$. Functional forms of the linear (red) and quadratic (orange) lines are shown in the legend. Panel a) shows the underlying data of all data points as grey logarithmic density and the global fit to them as red dashed line. Panel b) shows the deviation of data from a linear gradient as logarithmic density plot, whereas panel c) shows the 16th and 84th percentile around the median deviation as error bars in $\Delta R_\mathrm{Gal} = 0.5\,\mathrm{kpc}$ bins.}
...
\subsubsection{Local deviations from a global gradient} \label{sec:local_deviations_from_global_gradient}
...
see Fig.~\ref{fig:overlap_local_variation_gas}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figures/overlap_local_variation_gas.png}
    \caption{Local gradient deviations from bottom row of Fig.~\ref{fig:radial_range_impact_quadratic} overlapped on logarithmic density distribution of gas in $R-z$.}
